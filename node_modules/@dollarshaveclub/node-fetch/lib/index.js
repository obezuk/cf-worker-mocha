...
var es6 = require('@mattiasbuelens/web-streams-polyfill/ponyfill/es6');
...
function getTypeOfBody(body) {
	if (body == null) {
		...
		return "Stream";
	} else if (body instanceof es6.ReadableStream ||
...
function createReadableStream(instance) {
	...
	if (bodyType === 'ReadableStream') {
		return body.pipeThrough(new es6.TransformStream({
	...
	const readable = new es6.ReadableStream({
		start(controller) {
			switch (bodyType) {
				case "String":
					// body is a string:
...
function cloneBody(instance) {
	...
	if (bodyType === "Stream") {
		// tee instance body
		let p1 = new Stream.PassThrough();
		let p2 = new Stream.PassThrough();
